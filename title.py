...
def get_isp(ip):
    try:
        ip_geo_info = requests.get(f'http://ip-api.com/json/{ip}')
        ip_geo_info_dict = json.loads(ip_geo_info.text)
        return ip_geo_info_dict["isp"].split(",")[0].lower()
    except:
        return None
    ...
    if protocol_type == 'SHADOWSOCKS':
        ...
        for element in array_configuration:
            ...
            for ip_address in ips_list:
                ...
                # Modify configuration title based on server and protocol properties
                config["title"] = f"\U0001F512 VM-{config_type}-{config_secrt} {country_flag} {country_code}-{config['ip']}:{config['port']} \U0001F4E1 PING-{config_ping:06.2f}-MS"
        ...
        for element in array_configuration:
            ...
            for ip_address in ips_list:
                ...
                # Modify configuration title based on server and protocol properties
                config["title"] = f"\U0001F512 VL-{config_type}-{config_secrt} {country_flag} {country_code}-{config['ip']}:{config['port']} \U0001F4E1 PING-{config_ping:06.2f}-MS"
        ...
        for element in array_configuration:
            ...
            for ip_address in ips_list:
                ...
                # Modify configuration title based on server and protocol properties
                config["title"] = f"\U0001F512 TUIC-UDP {country_flag} {country_code}-{config['ip']}:{config['port']} \U0001F4E1 PING-{config_ping:06.2f}-MS"
        ...
        for element in array_configuration:
            if element.startswith('hysteria'):
                ...
                for ip_address in ips_list:
                    ...
                    # Modify configuration title based on server and protocol properties
                    config["title"] = f"\U0001F512 HYSTERIA-UDP {country_flag} {country_code}-{config['ip']}:{config['port']} \U0001F4E1 PING-{config_ping:06.2f}-MS"
...
def create_country(array_configuration):
    ...
    for config in array_configuration:
        if config.startswith('vless') or config.startswith('trojan') or config.startswith('ss'):
            country_code = config.split(' ')[-3].split('-')[0]
        ...
        if config.startswith('vmess'):
            ...
            config_title = dict_params.get('ps')
            country_code = config_title.split(' ')[-3].split('-')[0]
            country = country_code.lower()
            country_config_tp = (country, config)
            country_based_init_list.append(country_config_tp)
...
def create_internet_protocol(array_configuration):
    ...
    for config in array_configuration:
        ...
        if config.startswith('vmess'):
            ...
            config_title = dict_params.get('ps')
            ip_port = config_title.split(' ')[-3].split('-')[-1]
            if '[' in ip_port or ']' in ip_port:
                internet_protocol_ver6.append(config)
            else:
                internet_protocol_ver4.append(config)
